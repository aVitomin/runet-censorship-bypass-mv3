...
description: Prepare or audit a local Chromium MV3 release preflight for this repository, including version consistency, maintained tests, MV3 builds, local packaging, hashing, secret/private-URL checks, staged-output checks, and a release summary; do not trigger for ordinary development builds or when no release artifact is requested.
---
...
3. Run the maintained deterministic checks and build. Historical MV2 is not a current release target:
...
6. Return version/version-name, local preflight archive path relative to the project, SHA-256, commands and results, dirty-tree state, supply-chain checks, a concise change/security summary, and remaining browser QA. Label the archive as local preflight output, not a public release source. Browser QA includes load-unpacked startup/restart, provider refresh without auto-enable, Proxy/Auto/Direct, real Tor and authenticated proxy behavior, proxy errors/takeover, IndexedDB persistence, and upgraded-profile migration.
